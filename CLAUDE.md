...
- **xG and xT are allowed, as named models, under the rules below.** This reverses the original ban. That rule was written for an app whose entire subject was deliveries and first contacts, where danger reads from location and outcome alone. It stopped holding the moment the question became "how much of this team's threat came from dead balls", which is a question about quantity and cannot be answered with position.
  ...
  What is served is **a specific model, never "xG" in the abstract**: fitted on **9,718 matches across the big five leagues**, 2018-19 to 2025-26, 239,544 non-penalty shots, 24,000 goals. Out-of-fold log loss 0.2766, AUC 0.773, aggregate calibration 0.9999.

  **All five leagues are validated; the Champions League is not.** `lib.xg.validated_for` answers this, and the honest options remain to validate or to withhold — not to let the number appear anyway.

  **The model carries a per-league intercept, and it is not cosmetic.** A single shared fit looked perfectly calibrated overall while being biased by competition: it under-predicted Bundesliga by 6.2% and over-predicted Serie A by 3.9%. Five intercepts bring every league to within 0.5% at no cost to discrimination. Penalties are also fitted per league — 0.776 in Serie A against 0.811 in the Premier League, a wider spread than expected. A match whose competition cannot be identified takes the mean of the five rather than silently inheriting La Liga's.

  **More data barely moved the shape, and that is worth knowing before anyone gathers more.** Holding out a whole season: 291 matches gives log loss 0.2727 and AUC 0.760, all 1,896 gives 0.2714 and 0.762. Eleven coefficients saturate early — this is parameter-limited, not data-limited. A gradient booster on the same features reaches 0.2691 / 0.767, and was declined because it would put sklearn and a pickled artefact back into the serving path `lib/xg.py` exists to keep free of them.

  **Pooling leagues did not improve accuracy, only calibration.** A La Liga-only fit matched or beat the all-league fit on every one of the five, and beat three of the four own-league fits. Shot quality transfers across the big five; only the base rate does not.
